...
res_dwpli = zeros(combo_size, nofreqs, typename);
res_scoh = zeros(combo_size, nofreqs, typename);

res_chan = zeros(combo_size,1, typename);
res_chan2 = zeros(combo_size,1, typename);
...
try
    ...
    HasParProgress = true;
catch, warning('Missing ParFor Progressbar');
    HasParProgress = false;
end
...
parfor ci = 1 : combo_size
    ...
    % initialize
    ispc    = zeros(length(freqs2use),EEG.pnts, typename);
    pli     = zeros(length(freqs2use),EEG.pnts, typename);
    wpli    = zeros(length(freqs2use),EEG.pnts, typename);
    dwpli   = zeros(length(freqs2use),EEG.pnts, typename);
    scoh    = zeros(length(freqs2use),EEG.pnts, typename);
...
% convert back to CPU from GpuArrays
if ip.Results.gpuon
    res_ispc = gather(res_ispc);
    res_wpli = gather(res_wpli);
    res_dwpli = gather(res_dwpli);
    res_scoh = gather(res_scoh);
    res_chan  = gather(res_chan);
    res_chan2 = gather(res_chan2);
    note('Extracting from GpuArrays ...')
end

% create graphs for each frequency and measure
note('Generating BCMs per Frequency into 3D array...')
for fi = 1 : numel(frex)
...
tic; % .06 seconds
note('Compiling into export tables ...');
...
if ~ismissing(ip.Results.threshold)
    ...
    switch ip.Results.threshold % per frequency
        case 'mediansd'  % each frequency thresholded separately
            ...
            note('Calculating graph measures (thresholded) ...');
