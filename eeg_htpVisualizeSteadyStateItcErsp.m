function [EEGcell, results] = eeg_htpVisualizeSteadyStateItcErsp( EEGcell, varargin )
% Description: Creates ERP plots from eeg_htpCalcSteadyStateItcErsp.
% ShortTitle: Visualize assr ERP analysis
% Category: Analysis
% Tags: ERP
%
% Usage:
%    >> [ EEGcell, results ] = eeg_htpVisualizeSteadyStateItcErsp( EEGcell )
%
% Require Inputs:
...
if ip.Results.averageByRegion
    for ei = 1 : numel(EEGcell)
        EEGcell{ei} = eeg_htpAverageStructByRegion( EEGcell{ei}, 'scriptname','eeg_htpVisualizeSteadyStateItcErsp');
    end
end
...
if ndims(EEGcell{1}.vhtp.eeg_htpCalcSteadyStateItcErsp.itc1) > 2
    isMultiChannel = true;
    chanlabels = {EEGcell{1}.chanlocs.labels};
    for ei = 1 : numel(EEGcell)
        chanItc{ei}  = EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.itc1;
        chanErsp{ei} = EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.ersp1;
        chanStp{ei}  = EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.stp1;
    end
else
...
for ci = 1 : numel(chanlabels)
    ...
    if isMultiChannel
        for ei = 1 : length(EEGcell)
            ...
            EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.itc1 = chanItcNow(:,:,ci);
            EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.ersp1 = chanErspNow(:,:,ci);
            EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.stp1 = chanStpNow(:,:,ci);
        end
    end
    ...
    t = EEGcell{1}.vhtp.eeg_htpCalcSteadyStateItcErsp.t_s;
    f = EEGcell{1}.vhtp.eeg_htpCalcSteadyStateItcErsp.f_s;
    ...
    if ip.Results.groupmean  % single mean across groups
        ...
        for ei = 1 : length(EEGcell) % all ERPs in single array
            itcArr(:,:,ei) = EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.itc1;
            erspArr(:,:,ei) = EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.ersp1;
            stpArr(:,:,ei) = EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.stp1;
        end
        ...
        for gi = 1 : group_no % mean each by group id
            ...
            plot_filename_cell{gi} = fullfile(outputdir, ...
                ['assr_itcersp_by_group_' channame '_' num2str(groups(gi)) '_' timestamp '.png']);
        ...
        if ~isempty(ip.Results.contrasts)
            ...
            for contrast_i = 1 : numel(contrasts)
                ...
                plot_title_cell{group_no} = sprintf('PLACEHOLDER for group diff\n%s_%s (%s)',num2str(select_contrast{1}), num2str(select_contrast{2}), channame);
                plot_filename_cell{group_no} = fullfile(outputdir, ...
                    ['assr_itcersp_by_groupdiff_' channame '_' sprintf('%s_%s', num2str(select_contrast{1}), num2str(select_contrast{2})) '_' timestamp '.png']);
            end
        else
            ifDiff = false;
        end
        ...
        for ei = 1 : length(EEGcell)
            if ip.Results.singleplot && ei == 1 && ~ip.Results.groupmean
                ...
                plot_filename = fullfile(outputdir,['assr_itcersp_by_recording_' channame '_' timestamp '.png']);
                ...
                plot_filename_cell{ei} = fullfile(outputdir, ...
                    [temp_plot_name 'assr_itc' channame '.png']);
            ...
            itc(:,:,ei) = EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.itc1;
            ersp(:,:,ei) = EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.ersp1;
            stp(:,:,ei) = EEGcell{ei}.vhtp.eeg_htpCalcSteadyStateItcErsp.stp1;
        end
    ...
    if ip.Results.singleplot
        for gi = 1 : group_no
            ...
            subplot(1,3,1)
            createPlot_assrItc(t, f, itc(:,:,gi), ...
                strrep(plot_title_cell{gi},'PLACEHOLDER','ITC'));
            subplot(1,3,2)
            createPlot_assrStp(t, f, stp(:,:,gi), ...
                strrep(plot_title_cell{gi},'PLACEHOLDER','STP'));
            subplot(1,3,3)
            createPlot_assrErsp(t, f, ersp(:,:,gi), ...
                strrep(plot_title_cell{gi},'PLACEHOLDER','ERSP'));
            sgtitle(strrep(plot_title_cell{gi},'PLACEHOLDER','assr Plots'));
        ...
        for si = 1 : size(itc,3)
            disp(si);
            figure('Position', [600 600 1200 700], 'Visible', 'off');
            subplot(1,3,1)
            ...
            createPlot_assrItc(t, f, itc(:,:,si), ...
                ['ITC ' addtitle]);
            subplot(1,3,2)
            createPlot_assrStp(t, f, stp(:,:,si), ...
                ['STP ' addtitle]);
            subplot(1,3,3)
            createPlot_assrErsp(t, f, ersp(:,:,si), ...
                ['ERSP ' addtitle]);
            sgtitle(strrep(plot_title_cell{si},'PLACEHOLDER','assr Plots'));
...
function fig = createPlot_assrItc(t, f, itc, plot_title)
...
function fig = createPlot_assrStp(t, f, stp,plot_title)
...
function fig = createPlot_assrErsp(t, f, ersp,plot_title)
