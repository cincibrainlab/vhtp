function [EEG, results] = eeg_htpCalcSteadyStateItcErsp(EEG, varargin)
...
%
% Usage:
%    >> [ EEG ] = eeg_htpCalcSteadyStateItcErsp( EEG, varargin )
%
% Require Inputs:
...
%  eeg_htpCalcSteadyStateItcErsp(EEG, 'sourceOn', true, 'byChannel', true, 'baselinew', [-500 0]);
%
...
defaultTLimits = [-1000 4000];
defaultBaselineW = [-1000 0];
...
for ci = 1 : size(data,1)
    ...
    if plotroi
        ...
        % Define the ROI for ITC40
        hz_indices = roi.ITC40_hz{1};  % Indices of frequencies for ITC40 (e.g., 39-41 Hz)
        ms_indices = roi.ITC40_ms{1};  % Indices of time for ITC40 (e.g., 600-2400 ms)

        % Get the frequency and time limits for the box
        f_min = f_s(hz_indices(1));  % Minimum frequency in the ROI
        f_max = f_s(hz_indices(end)); % Maximum frequency in the ROI
        t_min = t_s(ms_indices(1));   % Minimum time in the ROI
        t_max = t_s(ms_indices(end));  % Maximum time in the ROI
        ...
        rectangle('Position', [t_min, f_min, t_max - t_min, f_max - f_min], ...
            'EdgeColor', 'r', 'LineWidth', 2);  % Red box with a line width of 2
        hold off;
...
% QI Table
qi_table = cell2table({EEG.filename, functionstamp, timestamp}, 'VariableNames', {'eegid', 'scriptname', 'timestamp'});

% Outputs:
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.itc1 = cat(3,chanItc{:});
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.rawitc1 = cat(3,chanRawItc{:});
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.ersp1 = cat(3,chanErsp{:});
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.stp1 = cat(3,chanStp{:});
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.t_s = t_s;
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.f_s = f_s;
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.summary_table = csvTable;
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.qi_table = qi_table;
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.trials = EEG.trials;
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.amp_rej_trials = num2str(bad_trial_idx);
EEG.vhtp.eeg_htpCalcSteadyStateItcErsp.amp_threshold = amp_threshold;
results =  EEG.vhtp.eeg_htpCalcSteadyStateItcErsp;
