...
fm1 = ff.FOOOF(aperiodic_mode='fixed', max_n_peaks=5,peak_width_limits=[1.0, 8.0])
fm2 = ff.FOOOF(aperiodic_mode='knee', max_n_peaks=5,peak_width_limits=[1.0, 8.0])

fm1.print_settings(description=True)
fm2.print_settings(description=True)

# Set a frequency range and add the data to the object
freq_range = [2, 40]
fm1.add_data(freqs, psd_chans[2,:], freq_range)
fm2.add_data(freqs, psd_chans[2,:], freq_range)

# You can also unpack all fit parameters when using `get_results`
ap_params, peak_params, r_squared, fit_error, gauss_params = fm1.get_results()
ap_params, peak_params, r_squared, fit_error, gauss_params = fm2.get_results()

fres = fm1.get_results()
fres.plot_fm()
fres = fm2.get_results()
...
# Initialize model object
fm1 = ff.SpectralModel(aperiodic_mode='fixed', max_n_peaks=5,peak_width_limits=[1.0, 8.0])
fm2 = ff.SpectralModel(aperiodic_mode='knee', max_n_peaks=5,peak_width_limits=[1.0, 8.0])

# Parameterize the power spectrum, and print out a report
fm1.report(freqs,  psd_chans[2,:], freq_range)
fm2.report(freqs,  psd_chans[2,:], freq_range)

import fooof as ff
ff.plts.fm.plot_fm(fm1,plot_aperiodic=True,plot_peaks=True, freq_range=[3, 40])
ff.plts.fm.plot_fm(fm2,plot_aperiodic=True,plot_peaks=True, freq_range=[3, 40])
plt.show()

specparam.plts.fm.plot_fm(fm1,plot_aperiodic=True,plot_peaks=True, freq_range=[3, 40])

fm3 = SpectralModel(aperiodic_mode='knee', max_n_peaks=6)
fm4 = SpectralModel(aperiodic_mode='knee', max_n_peaks=6)
