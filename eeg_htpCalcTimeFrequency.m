function [EEG, results] = eeg_htpCalcTimeFrequency(EEG, varargin)
...
%%
cfg.pnts = EEG.pnts;
cfg.tlimits = [EEG.times(1) EEG.times(end)];
cfg.Fs = EEG.srate;
cfg.cycles = cycles;
cfg.winsize = winsize;
cfg.nfreqs = nfreqs;
cfg.freqs = flimits;
cfg.baseline = ip.Results.baselinew;
cfg.timesout = timesout;
t_s = [];
f_s = [];
tf_by_chan = cell(EEG.nbchan,1);
% if ip.Results.byChannel
...
function [stp, ersp, itc, t_s, f_s] = calctf( datax, cfg )
cfg.winsize = 279;
cfg.nfreqs = 100;
cfg.cycles = [3 30];
% cfg.cycles =0;
...
[stp, itc, ~, t_s, f_s] = newtimef(double(datax), ...
    cfg.pnts, ... % frames
    cfg.tlimits, ... %tlimits
    cfg.Fs, ... %Fs
    cfg.cycles, ... % varwin (cycles)
    'winsize', cfg.winsize, ...
    'nfreqs', cfg.nfreqs, ...
    'freqs', cfg.freqs, ...
    'plotersp', 'on', ...
    ...
    'freqscale', 'log', ...
    'baseline', NaN, ...
    'timesout', cfg.timesout);

[ersp, ~, ~, t_s, f_s] = newtimef(double(datax), ...
    cfg.pnts, ... % frames
    cfg.tlimits, ... %tlimits
    cfg.Fs, ... %Fs
    cfg.cycles, ... % varwin (cycles)
