...
%KYLE CHANGE
%Changes to parfor loop by initializing time vector ahead of execution of loop and
%spectralevents_find method
tic;
parfor subj_j=1:numSubj
    ...
    for trl=1:size(X{subj_j},2)
        ...
        %Modified code without collection of tvec output
        [TFR_trl,~,~] = spectralevents_ts2tfr(X{subj_j}(:,trl),fVec,Fs,7); %Transformation calculated using a Morlet wavelet (width=7), see 4DToolbox for function details)
