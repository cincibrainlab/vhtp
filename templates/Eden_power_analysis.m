...
for i=1:length(all_results)
    ...
    partial_ID = strsplit(partial_filename{1},{'D','_'});
    ID(i) = str2double(strsplit(partial_ID{2},'D'));
    spectrum(:,i) = all_results{i}.pow.spectro.relpow; % mean-chan spectrum
end
...
subplot(121);plot(freq(1:30), log(spectrum_NH(1:30,:)), 'color', [0, 0.4470, 0.7410, 0.3])
hold on;plot(freq(1:30), mean(log(spectrum_NH(1:30,:)),2), 'color', 'r','linewidth',1.2);xlim([1 17]);ylim([-7 -1])
...
subplot(122);plot(freq(1:30), log(spectrum_HL(1:30,:)), 'color', [0, 0.4470, 0.7410, 0.3])
hold on;plot(freq(1:30), mean(log(spectrum_HL(1:30,:)),2), 'color', 'r','linewidth',1.2);xlim([1 17]);ylim([-7 -1])
...
% theta through alpha2 (6.5-11 Hz), p=0.1396

[h, p] = vartest2(spectrum_NH(find(freq==13),:), spectrum_HL(find(freq==13),:), 'Tail', 'right') % F-test
p = vartestn([[spectrum_NH(find(freq==13),:)'; NaN; NaN], spectrum_HL(find(freq==13),:)'], 'TestType', 'LeveneQuadratic') 
%           F-test (normal)   Levene test (nonnormal)
% 8Hz       p=0.0193            x
% 8.5Hz     p=0.0043            x
% 9Hz       p=0.0017            p=0.0493
% 9.5Hz     p=0.0101            x
% 10Hz      p=0.0122            x
% 10.5Hz    p=0.0328            x
% 11Hz      p=0.0195            x
% 11.5Hz    p=0.0047            x
% 12Hz      p=0.0046            x
% 12.5Hz    p=0.0071            p=0.0322
% 13Hz      p=0.011             p=0.0258
